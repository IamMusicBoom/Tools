 ...
 */
public class BModel extends BaseModel<IAllApi> {
    ...
    public void getDatas(String type, final HttpCallBack<ResultBean> callBack) {
        ...
        Observable<BModel> news = iAllApi.getNews(type, map);
        news.subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<BModel>() {
                    ...
                    public void onNext(BModel bModel) {
                        ...
                        } else if (bModel.getResult().getData() == null) {
                            callBack.onFail("返回数据为空...");
                        } else if (bModel.getResult().getData().size() <= 0) {
                            callBack.onFail("返回数据为空...");
                        } else {
                            callBack.onSuccess(bModel.getResult());
                        }

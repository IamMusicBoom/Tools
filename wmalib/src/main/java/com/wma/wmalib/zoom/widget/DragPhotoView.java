package com.wma.wmalib.zoom.widget;
...
public class DragPhotoView extends PhotoView {
    ...
    private final static int MAX_EXIT_Y = 0;
...
//        Math.abs(x2 - x1) + "\t\t\ty：" +  Math.abs(y2 - y1)
        if (Math.abs(mTranslateY - mScaleY) < 50 && Math.abs(mScaleX - mScaleX) < 50) {
